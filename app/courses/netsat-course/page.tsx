...
export default function NetsatCoursePage() {
  ...
  const { courses, loading: loadingCourses } = useRecommendedCourses("คอร์ส-เคมีNETSAT")
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const params = new URLSearchParams({ postType: "วิดีโอแนะนำ-NETSAT", limit: "1" })
    ...
    ;(async () => {
      try {
        ...
        while (!cancelled && loops < 20) {
          const params = new URLSearchParams({ postType: "ภาพสรุป-NETSAT", limit: String(pageSize), page: String(page) })
          const res = await fetch(`/api/posts?${params.toString()}`, { cache: "no-store" })
          const json = await res.json().catch(() => ({}))
    ...
    <section className="min-h-screen bg-gradient-to-br from-background to-accent">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          <h1 className="text-4xl lg:text-5xl font-bold text-foreground mb-4 text-balance">คอร์สเคมี NETSAT</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto text-pretty">
            เตรียมความพร้อมสำหรับการสอบ NETSAT เคมี ด้วยเนื้อหาที่ครอบคลุมและเทคนิคการทำข้อสอบ
          </p>
        </div>
